...
if ($flag == true) {
    if (checkStudent($oid) == 1) {
        ...
        if (password_verify(test_input($_POST["password"]), $student['password'])) {
            if ($remeber == true) {
                setcookie("usertype", "student", time() + 5 * 3600, '/');
            } else
            ...
            $wrong = ' <div class="alert alert-danger"> <strong>Wrong Credentials!</strong> The id or password is incrroect </div>;';
    } else if (checkTeacher($oid) == 1) {
        $teacher = viewTeacher($_POST['oid']);
        if (password_verify(test_input($_POST["password"]), $teacher['password'])) {
            if ($remeber == true) {
                setcookie("usertype", "teacher", time() + 5 * 3600, '/');
            } else
